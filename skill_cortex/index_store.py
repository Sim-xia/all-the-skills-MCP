...
def load_index(cache_path: Path) -> ScanResult | None:
	...
	for item in items:
		if not isinstance(item, dict):
			continue
		skill_path = Path(str(item.get("skill_path", "")))
		...
		try:
			...
			return None
		if item.get("skill_mtime_ns") != stat.st_mtime_ns:
			return None
		if item.get("skill_size") != stat.st_size:
			return None
